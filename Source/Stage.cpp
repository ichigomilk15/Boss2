...
Stage::Stage() :
	...
	rockModel(std::make_shared<Model>("./Data/Model/Stage/SquareBorder.mdl")),//todo : modelの読み込み変更
	cardModel()
{
...
void Stage::SearchSquare(const int x, const int y, const int cost, std::vector<DirectX::XMINT2>& squaresChecked, const std::vector<Square::Type>& typesExclusion)
{
	...
	SearchSquare(x, y - 1, cost - 1, squaresChecked, typesExclusion);      // 上

	SearchSquare(x, y + 1, cost - 1, squaresChecked, typesExclusion);      // 下

	SearchSquare(x - 1, y, cost - 1, squaresChecked, typesExclusion);      // 左

	SearchSquare(x + 1, y, cost - 1, squaresChecked, typesExclusion);      // 右
}
