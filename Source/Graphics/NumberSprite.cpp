...
{
	const DirectX::XMFLOAT2 texNumSize = { numbers.GetTextureWidthf() / 14.0f,numbers.GetTextureHeightf() };
	...
	{ 
		...
		{
			DirectX::XMFLOAT2 texpos = { texNumSize.x * 12,texNumSize.y };
			numbers.Render(dc, nowPos, numSize, texpos, texNumSize, .0f, color);
		}
		else if (num == '/')
		{
			DirectX::XMFLOAT2 texpos = { texNumSize.x * 13,texNumSize.y };
			numbers.Render(dc, nowPos, numSize, texpos, texNumSize, .0f, color);
		}
		else if('0'<=num&&num<='9')
		{
